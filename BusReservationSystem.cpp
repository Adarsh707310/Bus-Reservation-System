...
///***************************************FUNCTION TO LOAD DATA FROM CSV FILE TO VEVTOR OF BUSES*********************************************************///
    ...
    void Bus::load_buses(){                                                             ///Function is loading all the data from a csv file(bus_data.csv)
        ...
        std::string x;
        getline(u,x,',');
        temp->available_seats=bus::stoi(x);                                                  ///Assigning value from file to available_seats of current object by converting into integer
        getline(u,x,',');
        temp->reserved_seats=bus::stoi(x);                                                   ///Assigning value from file to reserved_seats of current object by converting that into integer
        getline(u,x,',');
        temp->total_seats=40;

        for(int i=0;i<39;i++){                                                          ///Assigning value from file to seats of current object seats into vector of seats
         getline(u,x,',');
         temp->vec_seats[i]=bus::stoi(x);                                                    ///Converting seat number into integer and saving in current temporary object
        }
          getline(u,x,'\n');
         temp->vec_seats[39]=bus::stoi(x);
...
///******************************************UTILITY FUNCTION TO CHECK THE VALIDITY OF DATE ENTERED BY USER OR ADMIN************************************///
    ...
    try{
        ...
        if(y>bus::stoi(str.substr(6,4))||(y==bus::stoi(str.substr(6,4)) && m>bus::stoi(str.substr(3,2)) )||(y==bus::stoi(str.substr(6,4)) && m==bus::stoi(str.substr(3,2)) &&d>bus::stoi(str.substr(0,2)) ) ){
        ...
        if(str.size()!=10){
         ...
         //   int y = stoi(s3);
                ...
                if(str[2] == '/' && str[5] == '/'){
                    if( ( m==2 && d>=1 && d<=29 && leap(y) ) || ( m==2 && d>=1 && d<=28 && !leap(y) ) ){
